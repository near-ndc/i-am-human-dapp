...
async function addAccountToStatus(accountID) {
...
describe("ManageAdmin", () => {
  ...
  test("should show admin status with correct account ID", async () => {
     await addAccountToStatus(accountIDWithoutAdminStatus)
      wallet.viewMethod.mockResolvedValue(false);
      await waitFor(() => {
    ...
    test("removes admin successfully", async () => {
     await addAccountToStatus(accountIDWithAdminStatus)
     wallet.viewMethod.mockResolvedValue(true);
      const removeAdmin = () => {};
